...
FROM fn_mostraJogos()
****ATUALIZAR A PROCEDURE sp_geraJogos****
***FAZER UM BOTÃO QUE CHAME A PROCEDURE sp_mudaFinal E RETORNE OS JOGOS (ESSA PROCEDURE VAI RANDOMIZAR OS GOLS)
***
*/
USE Testes
EXEC sp_mudaFinal
SELECT * FROM JOGOS
CREATE PROCEDURE sp_mudaFinal
AS
BEGIN
	...
	BEGIN
		SET @gol1 = RAND()*4
		SET @gol2 = RAND()*4
		SET @query = 'UPDATE Jogos SET gols_TimeA = '+CAST(@gol1 AS VARCHAR)+
		', gols_TimeB = '+CAST(@gol2 AS VARCHAR)+'WHERE id_Jogo = '+CAST(@cont AS VARCHAR)
		EXEC(@query)
		SET @cont = @cont + 1
	END
END
...
/*Criar a view que começa na linha 427
SELECT nomeTime AS Time, jogosD AS 'Jogos Disputados', vitorias AS 'Vitorias', 
empates AS 'Empates', derrotas AS 'Derrotas', golsM AS 'Gols Marcados',
...
BEGIN
	...
	BEGIN
		...
		+CAST(0 AS VARCHAR)+', '+CAST(0 AS VARCHAR)+', '''
